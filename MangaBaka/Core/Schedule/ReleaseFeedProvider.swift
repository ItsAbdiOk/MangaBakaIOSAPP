...
/// private-API rule (`AppServices.swift`), and everything that consumed it —
/// `TranslationGap`, `ReleaseFeed.totalCount` and `.finished`, and the
/// Naver-finished notification condition — was deleted on 2026-09-14 rather
/// than left standing over a populator that no longer exists. See the
/// tombstone in `ReleaseFeedService.swift`.
///
...
protocol ReleaseFeedProvider: Sendable {
    ...
    /// season-ended feed a week stale is still season-ended, and
    /// `ReleaseReminders.reschedule` only needs to notice a change since the
    /// last time it looked, not a fresh number.
    ///
    ...
    /// season-ended notification conditions can fire from whatever a prior
    /// series-page visit already cached, without adding a single network call
    /// of its own.
