...
struct AppleBooksMatchTests {
    ...
    /// Omniscient Reader's Viewpoint, checked against the live GB and US
    /// stores 2026-09-14: Apple Books carries only the novel — "Omniscient
    /// Reader's Viewpoint (novel), Vol. 1"…5 — and none of Ize Press's 13
    /// manhwa volumes. The manhwa's shelf must therefore be *empty*, not
    /// five novel covers under comic numbers. "(novel)" sits between the
    /// title and the marker, which the tag rule refuses for the comic — and,
    /// until 2026-09-15, refused for the novel too: the pattern allowed a
    /// bracket before the marker only when whitespace followed it, so
    /// ", Vol. 1" made the lazy title swallow "(novel)" and the novel's own
    /// page had no volumes either. Second half failed with `== []`.
    @Test("A manhwa the store only sells as a novel gets an empty shelf, not the novel's covers")
    func novelOnlyStoreLeavesTheComicShelfEmpty() {
        let results = (1...5).map { number in
            result(number, "Omniscient Reader's Viewpoint (novel), Vol. \(number)", by: "singNsong")
        }
        let comic = AppleBooksMatch.volumes(
            in: results, titles: ["Omniscient Reader's Viewpoint"], creators: ["singNsong"], isNovel: false
        )
        #expect(comic.isEmpty, "\(comic.map(\.number)) — the novel's volumes on the manhwa's shelf")
        // And the novel's own page does get them.
        let novel = AppleBooksMatch.volumes(
            in: results, titles: ["Omniscient Reader's Viewpoint"], creators: ["singNsong"], isNovel: true
        )
        #expect(novel.map(\.number) == [1, 2, 3, 4, 5])
    }
